...
| AUTH-02 | Publication requires approval bound to the exact native product target, copy, evidence set and evidence provenance. | Approval envelope and independently recomputed target-inclusive digest in `publishVariant` and the Shopify adapter boundary. | Cross-product, changed-copy, changed-provenance, stale-approval and illegal-transition tests. |
...
| EVID-03 | Version, native product identity, observation time and freshness survive ingestion into operational state and remain visible to WebMCP consumers. | `EvidenceRecord` is retained through `AppState`; evaluators reject wrong-product evidence. | Store and WebMCP contract assertions plus wrong-product evaluation counterexample. |
...
| ADS-01 | Ads preparation uses the exact approved publication, passes the bounded local feed-schema validator, and remains a PAUSED projection. | Digest/status guard, feed validator and fixed campaign status. | Precondition, independent invalid-row oracle, produced-row validation and zero-authority tests. |
