...
Protect the demonstrable business outcome: an agent can discover and improve a product journey, but verified merchant evidence and exact human approval remain authoritative and no live paid activation occurs.
...
- Use the Playwright smoke journey when a change can affect tool discovery, shared browser state, the merchant approval handoff, published shopper copy, Ads projection, or the cart.
...
The skill may interpret a change, propose scenarios, add tests, and explain failures. It does not decide whether CI passes, weaken thresholds to make a change green, approve a product variant, or authorize Shopify, checkout, payment, Ads API, campaign activation, or spend.
